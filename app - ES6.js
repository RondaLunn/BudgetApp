...
const UIController = (() => {
    ...
    const formatNumber = (num, type) => {
        ...
        int = numSplit[0];
        if (int.length > 3) {
            int = int.replace(/\B(?=(\d{3})+(?!\d))/g, `,`); //input 23510, output 23,510
        }
    ...
    return {
        ...
        changedType: () => {
            let {inputType,inputDescription,inputValue} = DOMstrings;
...
const controller = ((budgetCtrl, UICtrl) => {
    ...
    const ctrlAddItem = () => {
        ...
        if (input.description !== `` && !isNaN(input.value) && input.value > 0) {
          ...
          window.alert(`Error: Please Enter a Description`);
